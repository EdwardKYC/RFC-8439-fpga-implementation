...
module AES(clk,nrst,start,enc_dec,data_in,key_in,key_gen_start, key_gen_Done ,output_valid,data_out,addr, write_en, done);
input clk,nrst;
input start;
input key_gen_start;
input enc_dec;
...
begin
	...
	INPUT:begin
		if(cnt == 3'd4 && enc_dec == 1'd0) next_state = ENCRYPTION;
...
begin
	...
	begin
		...
		begin
			if(enc_dec==1'd0)
			begin
				data_out<=ENC_AddRoundKey_out;
			end
			else
			begin
				data_out<=DEC_AddRoundKey_out;
			end
		end
		else if(input_valid==1'd1)
		begin
